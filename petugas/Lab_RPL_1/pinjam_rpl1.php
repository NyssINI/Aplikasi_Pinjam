...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['pinjam'])) {
  ...
  if (!empty($nomor_laptop)) {
    mysqli_query($conn, "UPDATE lab_rpl1 SET status='Dipinjam' WHERE nomor_laptop='$nomor_laptop'");

    mysqli_query($conn, "INSERT INTO histori_peminjaman (nama_peminjam, kelas, jam_pinjam, jam_kembali, status, nomor_laptop, lab) 
      VALUES ('$nama_peminjam', '$kelas', '$jam_pinjam', '$jam_kembali', 'Dipinjam', '$nomor_laptop', 'RPL 1')");
  }
}
...
if (isset($_GET['selesai']) && isset($_GET['nomor_laptop'])) {
  ...
  if (!mysqli_query($conn, "UPDATE lab_rpl1 SET status='Tersedia' WHERE nomor_laptop='$nomor_laptop'")) {
  ...
  echo "<script>
      document.addEventListener('DOMContentLoaded', function() {
          ...
          Swal.fire({
              ...
              window.location.href='?page=pinjam_rpl1';
          });
      });
  </script>";
  exit();
        ...
        <tbody>
          ...
          while ($row = mysqli_fetch_assoc($result)) {
            echo "<tr class='text-center bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition'>";
              ...
              echo "<td class='border border-gray-300 dark:border-gray-600 p-2 text-green-600 dark:text-green-400 font-semibold'>" . $row['status'] . "</td>";
              ...
              echo "<td class='border border-gray-300 dark:border-gray-600 p-2 flex justify-center gap-2'>";
                if ($row['status'] == "Tersedia") {
                  ...
                  echo "<a href='?page=pinjam_rpl1&selesai&nomor_laptop=" . $row['nomor_laptop'] . "' class='bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600 dark:hover:bg-green-700 transition'>Selesai</a>";
                }
              echo "</td>";
            echo "</tr>";
          }
